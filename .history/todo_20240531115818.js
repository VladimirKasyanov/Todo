...
const updateCounters = () => {
  ...
  incompleteCounter.innerText = incompleteCount;
  totalCounter.innerText = todoList.length;
...
const completedTab = () => {
  const completedCount = todoList.filter((item) => item.checked).length;
  const incompleteCount = todoList.length - completedCount;
  totalCounter = todoList.length
  switch(allButton) {
    case (totalCounter = todoList.length):
      break;
    // case (completedCount = completedCounter):
    //   break;
    // case (incompleteCount = incompleteCounter):
    //   break;
...
const changeDescriptionFinish = (event) => {
  ...
  allButton.addEventListener('click',completedTab )
