...
<span class='description' for='${i}'>${item.description}</span>
...
const changeDescription = (event) => {
  if (event.target.classList.contains("description")) {
    ...
    event.children[2].hidden = true 
    event.children[3].hidden = false
    event.children[3].focus()
  }
};
...
const changeDescriptionFinish = (event) => {
    if (event.target.classList.contains("new-input")) {
      ...
      // const todoId = Number(event.target.parentElement.id)
         event.keyEnter 
        //  todoList.description = newValue;
        displayList(todoList);
      
        event.keyEscape 
        event.target.value = todoList.description;
