  ...
  arr.forEach((item, i) => {
    list.innerHTML += `
      ...
      item.checked ? "checked" : ""
            ...
            <button class='del' id='${item.id}' type='remove' onclick='remove'>x</button>
        </li>
    `;
  });
};
...
const removeOnce = (event) => {
  ...
  delButtons.forEach((button) => {
    button.addEventListener("click", removeOnce);
  });
// const removeOnce = (event) => {
//     const delButtons = todoList.findIndex((todo) => todo.id === event.target.id);
//     todoList.splice(index, 1)
//     // if (index !== -1) {}
